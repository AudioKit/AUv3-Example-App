...
public protocol AKKeyboardDelegate: AnyObject {
    /// Note on events
    func noteOn(note: MIDINoteNumber, velocity: MIDIVelocity, offset: MIDITimeStamp)
    /// Note off events
    func noteOff(note: MIDINoteNumber, offset: MIDITimeStamp)
}
...
@IBDesignable open class SynthKeyboard: UIView, AKMIDIListener {
    ...
    /// Activated key color
    @IBInspectable open var  keyOnUserColor: UIColor = #colorLiteral(red: 0.788718164, green: 0.7888327241, blue: 0.78869313, alpha: 1)
    var keyOnColor: UIColor =  #colorLiteral(red: 0.6666666667, green: 0.6666666667, blue: 0.6666666667, alpha: 1)
    ...
    var touchedNotes = Set<MIDINoteNumber>()
    ...
    var currentVelocity: MIDIVelocity = 80
    var velocitySetting = KeyboardVelocitySetting.fixed
    let verticalRange: ClosedRange = 0.0 ... 127.0
    let verticalTaper = 1.0
    ...
    // MARK: - Touch Handling
    func notesFromTouches(_ touches: Set<UITouch>) -> [NoteData] {
        var notes = [NoteData]()
        for touch in touches {
            
            if let note = noteFromTouchLocation(touch.location(in: self)) {
                let noteData = NoteData(noteNumber: note.noteNumber, noteVelocity: note.noteVelocity)
    ...
    func noteFromTouchLocation(_ location: CGPoint ) -> NoteData? {
        ...
        if y > oneOctaveSize.height * topKeyHeightRatio {
            ...
            noteNumber = (firstOctave + octNum) * 12 + whiteKeyNotes[max(0, Int(scaledX / whiteKeySize.width))] + baseMIDINote
        } else {
            let scaledX = x - CGFloat(octNum) * oneOctaveSize.width
            noteNumber = (firstOctave + octNum) * 12 + topKeyNotes[max(0, Int(scaledX / topKeySize.width))] + baseMIDINote
        }
        
        // Phantom C# fix
        let phantomCsharp = (octaveCount * 12 + 1) + (firstOctave * 12) + baseMIDINote
        if noteNumber == phantomCsharp {
            noteNumber = phantomCsharp - 1
        }
        
        if noteNumber >= 0 {
            var velocity = MIDIVelocity(getYWithTouchPoint(location, noteNumber: MIDINoteNumber(noteNumber)))
            if velocitySetting == .fixed { velocity = currentVelocity }
            return NoteData(noteNumber: MIDINoteNumber(noteNumber), noteVelocity: velocity)
        } else {
            return nil
        }
    ...
    override open func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        let notes: [NoteData] = notesFromTouches(touches)
        
        for note in notes {
            pressAdded(note.noteNumber, velocity: note.noteVelocity)
            touchedNotes.insert(note.noteNumber)
        }
        if !holdMode { verifyTouches(event?.allTouches) }
        setNeedsDisplay()
    }
    
    func getYWithTouchPoint(_ touchPoint: CGPoint, began: Bool = false, noteNumber: MIDINoteNumber) -> Double {
        var y: CGFloat = 0.0
        
        // determine if blackkey or white key
        var isBlackKey = false
        if !whiteKeyNotes.contains(Int(noteNumber % 12)) {
            isBlackKey = true
        }
        
        if isBlackKey {
            y = CGFloat((0.0 ... 1.0).clamp(1 - touchPoint.y / (oneOctaveSize.height * topKeyHeightRatio)))
        } else {
            y = CGFloat((0.0 ... 1.0).clamp(1 - touchPoint.y / bounds.size.height))
        }
        
        var verticalValue = (Double(y).denormalized(to: verticalRange, taper: verticalTaper))
        
        // top of keys 0, bottom of keys 127
        if velocitySetting == .normal {
            verticalValue = 127 - verticalValue
        }
        ...
        for touch in touches {
            if let note = noteFromTouchLocation(touch.location(in: self)) {
                ...
                if var otherTouches = event?.allTouches {
                    ...
                    if ❗️notesFromTouches(otherTouches).contains(where: { $0.noteNumber == note.noteNumber }) {
                        pressRemoved(note.noteNumber, touches: event?.allTouches)
                        touchedNotes.remove(note.noteNumber)
                    }
                }
            }
        }
    ...
    override open func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            if let note = noteFromTouchLocation(touch.location(in: self)),
                note.noteNumber != noteFromTouchLocation(touch.previousLocation(in: self))?.noteNumber {
                pressAdded(note.noteNumber, velocity: note.noteVelocity, offset: 0)
                touchedNotes.insert(note.noteNumber)
                setNeedsDisplay()
            }
        }
    ...
    func pressAdded(_ newNote: MIDINoteNumber, velocity: MIDIVelocity = 127, offset: MIDITimeStamp = 0,
                    sendToDelegate: Bool = true) {
        ...
        if ❗️onKeys.contains(newNote) && !noteIsAlreadyOn {
            ...
            if sendToDelegate {
                delegate?.noteOn(note: newNote, velocity: velocity, offset: 0) //fixme offset
            }
        }
        setNeedsDisplay()
    }
    ...
    func pressRemoved(_ note: MIDINoteNumber, touches: Set<UITouch>? = nil, sendToDelegate: Bool = true) {
        guard onKeys.contains(note) else {
            return
        }
        onKeys.remove(note)
        if sendToDelegate {
            delegate?.noteOff(note: note, offset: 0) //fixme offset
        }
        if ❗️polyphonicMode {
            // in mono mode, replace with note from highest remaining touch, if it exists
            var remainingNotes: [NoteData] = notesFromTouches(touches ?? Set<UITouch>())
            remainingNotes = remainingNotes.filter { $0.noteNumber != note }
            if let highest = remainingNotes.last {
                pressAdded(highest.noteNumber, velocity: highest.noteVelocity)
            }
        }
        setNeedsDisplay()
    }
    ...
    private func verifyTouches(_ touches: Set<UITouch>?) {
        ...
        let noteData = notesFromTouches(touches ?? Set<UITouch>() )
        var disjunct = touchedNotes
        for badNote in noteData {
            disjunct.remove(badNote.noteNumber)
        }
        if !disjunct.isEmpty {
            for note in disjunct {
                pressRemoved(note)
                touchedNotes.remove(note)
            }
        }
    }
    ...
    func whiteKeyColor(_ n: Int, octaveNumber: Int) -> UIColor {
        if darkMode {
            ...
            keyOnColor = #colorLiteral(red: 0.5333333333, green: 0.5333333333, blue: 0.5333333333, alpha: 1)
        } else {
    ...
    func topKeyColor(_ n: Int, octaveNumber: Int) -> UIColor {
        if darkMode {
            blackKeyOff = #colorLiteral(red: 0.3103210926, green: 0.3110416532, blue: 0.3322413266, alpha: 1)
            keyOnColor = #colorLiteral(red: 0.6, green: 0.6, blue: 0.6, alpha: 1)
        } else {
